...
export const TourOverlay = forwardRef<TourOverlayRef, TourOverlayProps>(
  (props, ref) => {
    ...
    const { goTo, next, previous, start, steps, stop, changeSpot } =
...
function makeFloatingOptions(
  arrowRef: RefObject<View>,
  props: Optional<TooltipProps>,
): UseFloatingOptions {
  const arrowOption =
    typeof props?.arrow === "boolean" ? DEFAULT_ARROW : props?.arrow;
  const { size } =
    typeof arrowOption === "number"
  ...
  const offsetValue =
    props?.arrow !== false
      ...
      : baseOffset;
  const arrowMw =
    props?.arrow !== false ? arrow({ element: arrowRef }) : undefined;
  const flipMw =
    props?.flip !== false
